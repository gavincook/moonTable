...
(function () {
    ...
    var methods = {
        ...
        renderDataRow: function (opts, index, level, columnData) {//渲染数据行
            ...
            var $tr = methods.ce("tr", {
                "data-number": index,
                "level": level,
                "data-id": "tr_" + columnData[opts.rowId] || index
            });//data-number表示当前行的序号,用户获取选择行使用,从0开始计数
            ...
            if (opts.showSelectBox) {//处理单选框或复选框
                ...
                var $selectBox = methods.ce("input", {
                    "name": "selectBox",
                    "type": (opts.multiSelect ? "checkbox" : "radio")
                });
            ...
            if (opts.showNumber) {//处理序号列
                ...
                if (methods.getLevel(level) == 1) {
                    if (opts.serialNumberScope === "all") {//序列号叠加
                        var lastIndex = (opts.currentDataSize == 0) ? 0 : ((opts.pageNum - 1) * opts.pageSize);//上一页最后的序列号
        ...
        renderHeader: function () {
            ...
            $.each(opts.columns, function (index, column) {//渲染数据列表头
                ...
                var $dataTh = methods.ce("th", {
                    "data-name": (column.sortName || column.name),
                    "class": (column.sort ? "sort-column" : ""),
                    "style": "text-align:" + (column.align || "left") + "; width:" + (column.width || "auto") + ";"
                })
        ...
        renderTable: function () {
            ...
            $.when(renderTableDfd).done(function () {
                ...
                !$.isFunction(opts.done) || opts.done.call(tableInstance);
            });
        ...
        renderPagination: function () {
            var tableInstance = this;
            var opts = tableInstance.opts;
            var startIndex = (opts.currentDataSize == 0) ? 0 : ((opts.pageNum - 1) * opts.pageSize + 1);
            ...
            $paginationDiv.append(methods.ce("span", {
                "class": "pagination-btn",
                action: "first"
            }).append(methods.ce("i", {"class": "fa fa-step-backward"})));
            $paginationDiv.append(methods.ce("span", {
                "class": "pagination-btn",
                action: "prev"
            }).append(methods.ce("i", {"class": "fa fa-play fa-rotate-180"})));
            $paginationDiv.append(methods.ce("input", {
                "class": "input-small",
                type: "text",
                name: "currentPage",
                value: (opts.pageNum || 1)
            }));
            ...
            $paginationDiv.append(methods.ce("span", {
                "class": "pagination-btn",
                action: "next"
            }).append(methods.ce("i", {"class": "fa fa-play"})));
            $paginationDiv.append(methods.ce("span", {
                "class": "pagination-btn",
                action: "last"
            }).append(methods.ce("i", {"class": "fa fa-step-forward"})));
            $paginationDiv.append(methods.ce("span", {"class": "split"}));
            $paginationDiv.append(methods.ce("span", {
                "class": "pagination-btn",
                action: "refresh"
            }).append(methods.ce("i", {"class": "fa fa-refresh"})));
        ...
        refreshPagination: function () {
            ...
            var pageCount = Math.ceil((opts.totalItemsCount || 1) / opts.pageSize);
            opts.pageCount = pageCount || 1;
            $pagination.find(":text[name='currentPage']").val(opts.pageNum || 1);
        ...
        getData: function () {
            ...
            if (opts.data) {
                ...
                $.getJsonData(opts.url, $.extend({_random: Math.random()}, {
                    pageNum: opts.pageNum,
                    pageSize: opts.pageSize
                }, opts.params)).done(function (result) {
                    var data;
                ...
                $(".pagination-btn", $container).click(function (event) {
                    switch ($(this).attr("action")) {
                        ...
                        case "prev"   :
                            ...
                            break;
                        case "next":
                            if (opts.pageNum + 1 > opts.pageCount) {
                                opts.pageNum = opts.pageCount;
                            } else {
                                opts.pageNum = opts.pageNum + 1;
                            }
                            methods.refresh.call(tableInstance);
    ...
    $.fn.table = function (opts) {
        ...
        if (typeof(opts) == "string") {
            ...
            tableInstance.opts = $.extend(true, {}, defaults, opts);
