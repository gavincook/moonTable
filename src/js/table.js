...
(function(){
	...
	var methods  = {
			...
			if(opts.treeTable){//树形表格
				$.each(opts.columns,function(index,columnDefinition){//处理数据行
					...
					if($.isFunction(columnDefinition.render)){
						...
						$data.append(methods.getColumnData.call($dataTd,columnData, columnDefinition.name));
				...
				$.each(opts.columns,function(index,columnDefinition){//处理数据行
					...
					if($.isFunction(columnDefinition.render)){
						...
						$dataTd.html(methods.getColumnData.call($dataTd,columnData, columnDefinition.name));//没有自定义渲染方法，默认通过column.name取值
					}
					$tr.append($dataTd);
				});
			}
		...
		//根据属性获取对应的值，支持多级获取，如"A.B.C",会获取数据中A属性的B属性中的C属性的值
		getColumnData:function(columnData, property){
		...
		renderTable:function(){
			var tableInstance = this;
			var $container = tableInstance.$container;
			var autoLayout = ($container.height() < 60);//是否自适应布局
			var opts = tableInstance.opts;
		...
		getData:function(){
			...
			var opts = tableInstance.opts;
			var dfd = $.Deferred();
            opts.pageIndex = opts.params.pageIndex || opts.pageIndex;
			if(opts.data){ 
				var data = opts.data;
		...
		bindEvents:function(){
            ...
            if(opts.showPagination) {
                ...
                //回车翻页
                $container.on("keyup","[name='currentPage']",function(e){
                    if(e.keyCode === 13){
                        var $pageInput = $(e.target);
                        var pageIndex = $pageInput.val();
                        if(pageIndex && (pageIndex = +pageIndex) > 0 && pageIndex <= opts.pageCount) {
                            opts.pageIndex = $pageInput.val();
                            methods.refresh.call(tableInstance);
                        }else{
                            alert("输入页码有误,请重新输入.");
                        }
		...
		renderButtonGroup:function(){
			...
			$.each(tableInstance.opts.buttons,function(index,button){
				...
				$btn.addClass(button.css||"btn btn-small btn-link").html(button.text);
				$btn.bind("click",function(){
					button.click.call(tableInstance,button,$btn);
				});
				$btnDiv.append($btn);
			});
			return $btnDiv;
